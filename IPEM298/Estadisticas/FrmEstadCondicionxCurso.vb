Public Class FrmEstadCondicionxCurso
    Dim dataContext As New DataClassesipem2DataContext
    Dim datacontextvistas As New DataClassesdevistasDataContext
    Private Sub FrmEstadCondicionxCurso_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
    ...
    Sub TipoGrafico(ByVal grafico As DataVisualization.Charting.SeriesChartType)
        '"SELECT id_curso, nombre_curso, condicion_textbox, Total From dbo.VistaEstCalifxAlu where id_curso = '" & I & "'"

        Dim sql = From s In datacontextvistas.VistaEstCalifxAlu Select s.id_curso, s.nombre_curso, s.condicion_textbox, s.Total Where id_curso = CInt(cboCurso.SelectedValue)
